...
interface DriverOrderDetailsSectionProps {
  order: Partial<DriverOrderDetails> &
    Pick<DriverOrderDetails, "order_number" | "total" | "address" | "created_at">;
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.06] px-3.5 py-3">
      ...
      <ul className="space-y-2">
        {lines.map((line, index) => (
          <li
            key={`${line}-${index}`}
            className="flex gap-2.5 text-sm leading-relaxed text-white/95"
          >
...
function ExpandedOrderDetails({
  ...
  return (
    <div className="space-y-3">
      ...
      <DetailRow label="Κατάσταση κουζίνας" value={formatDriverOrderStatus(order.status)} />

      {deliveryStage && (
        <DetailRow label="Κατάσταση διανομής" value={formatDriverDeliveryStage(deliveryStage)} />
      )}
        ...
        <ul className="mt-1.5 space-y-1 rounded-xl bg-black/20 px-3 py-2">
          {items.length === 0 ? (
            ...
            items.map((item, index) => (
              <li
                key={`${item.name}-${index}`}
                className="flex justify-between text-white/90 text-xs"
              >
                <span>
...
export function DriverOrderDetailsSection({
  ...
  return <ExpandedOrderDetails order={order} deliveryStage={deliveryStage} compact={compact} />;
}
